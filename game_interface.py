...
def run_comparative_simulation(scale="Small", duration=24*60):
    ...
    for strategy in strategies:
        ...
        while sim.current_time < duration:
            sim.update_status()

            sim.current_time += 1  # Add this line if the time isn't being incremented elsewhere
            
            # Print progress every 10%
            if sim.current_time % (duration // 10) == 0:
                print(f"  Progress: {sim.current_time}/{duration} minutes ({sim.current_time/duration*100:.0f}%)")
        ...
        # Store results
        results[strategy] = {
            "completion_rate": sim.stats["completion_rate"],
            "avg_waiting_time": sim.stats["avg_waiting_time"],
            "avg_charging_time": sim.stats["avg_charging_time"],
            "battery_swaps": sim.stats["battery_swaps"],
            "avg_robot_utilization": sim.stats["avg_robot_utilization"],
            "completed_count": sim.stats["completed_count"],
            "failed_count": sim.stats["failed_count"]
        }


        print(f"  Completed: {sim.stats['completed_count']}")
        print(f"  Failed: {sim.stats['failed_count']}")
        print(f"  Completion rate: {sim.stats['completion_rate']:.1f}%")
        print()
...
def generate_comparison_graphs(results, scale, filename="strategy_comparison"):
    ...
    plt.xticks(x, [strategy_names[s] for s in strategies], rotation=45)
    ...
    plt.xticks([x + width/2 for x in x1], [strategy_names[s] for s in strategies], rotation=45)
    plt.legend()
